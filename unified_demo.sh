...
phase2_cli_loop() {
    ...
    # Use venv's nova if available
    if [ -f ".venv/bin/nova" ]; then
        .venv/bin/nova fix . --pytest-args "tests/test_calculator.py -p no:pytest_httpbin" --max-iters 2 --timeout 120 | sed '/^Replaced file:/d; /Changes saved to branch/d; /preserved for PR/d; /pull\//d; /Working on branch/d; /Repository:/d'
