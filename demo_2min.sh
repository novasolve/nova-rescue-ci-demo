...
setup() {
    ...
    # Ensure we have pytest in the current environment
    if [ -f "$REPO_DIR/.venv/bin/pytest" ]; then
        export PATH="$REPO_DIR/.venv/bin:$PATH"
    elif [ -f "$REPO_DIR/venv/bin/pytest" ]; then
        export PATH="$REPO_DIR/venv/bin:$PATH"
    fi
...
phase2_cli_demo() {
    ...
    if [ -f ".venv/bin/python" ]; then
        .venv/bin/python -m pytest tests/test_calculator.py -v -p no:pytest_httpbin
    elif [ -f "venv/bin/python" ]; then
        venv/bin/python -m pytest tests/test_calculator.py -v -p no:pytest_httpbin
    else
        python -m pytest tests/test_calculator.py -v -p no:pytest_httpbin
    fi
    ...
    print_cmd "pytest tests/test_calculator.py -v -p no:pytest_httpbin"
    if [ -f ".venv/bin/python" ]; then
        .venv/bin/python -m pytest tests/test_calculator.py -v -p no:pytest_httpbin || true
    elif [ -f "venv/bin/python" ]; then
        venv/bin/python -m pytest tests/test_calculator.py -v -p no:pytest_httpbin || true
    else
        python -m pytest tests/test_calculator.py -v -p no:pytest_httpbin || true
    fi
    ...
    # Ensure we use the venv's nova if available
    if [ -f ".venv/bin/nova" ]; then
        .venv/bin/nova fix . --pytest-args "tests/test_calculator.py -p no:pytest_httpbin" --max-iters 2 2>&1 | \
            sed '/Could not create PR/d; /You can manually create a PR/d; /Replaced file:/d; /Changes saved to branch/d'
    elif [ -f "venv/bin/nova" ]; then
        venv/bin/nova fix . --pytest-args "tests/test_calculator.py -p no:pytest_httpbin" --max-iters 2 2>&1 | \
            sed '/Could not create PR/d; /You can manually create a PR/d; /Replaced file:/d; /Changes saved to branch/d'
    else
